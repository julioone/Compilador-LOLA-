...
class CheckProgramVisitor(ast.NodeVisitor):
	...
	def __init__(self):
		super(CheckProgramVisitor, self).__init__()
	...
	def visit_Modulo(self, node):
		node.tipo="MODULE"
		target = self.new_node(node, None, 'Box', 'lightgray')
		self.dot.add_node(target)
		...
		for field in getattr(node, "_fields"):
			...
			if isinstance(value, ast.AST):
				...
				targetHijo = self.new_node(None, value, 'Box')
	...
	def visit_DeclaracionConstante(self, node):
		if(self.VisitandoModulo):
			...
			node.tipo="TYPE-CONST"
		target = self.new_node(node, shape='Box', color='lightgray')
		self.dot.add_node(target)
	...
	def visit_DeclaracionVariableOUT(self, node):
		...
		target = self.new_node(node, None, 'triangle', 'lightgray')
		self.dot.add_node(target)
		self.PortsOut=True
		for field in getattr(node, "_fields"):
			value = getattr(node, field, None)
	...
	def visit_Tipo(self, node):
		...
		if(self.VisitandoModulo):
			...
			if(self.PortsIn):
				node.tipo="MODULE-PortsIN"
				target = self.new_node(node, shape='Box', color='lightgreen')
			if(self.PortsInOut):
				node.tipo="MODULE-PortsINOUT"
				target = self.new_node(node, shape='Box', color='orange')
			if(self.PortsOut):
				node.tipo="MODULE-PortsOUT"
				target = self.new_node(node, shape='Box', color='pink')
			if(self.PortsVar):
				node.tipo="MODULE-PortsVar"
				target = self.new_node(node, shape='Box', color='violet')
		else:
			if(self.PortsIn):
				node.tipo="TYPE-PortsIN"
				target = self.new_node(node, shape='Box', color='lightgreen')
	...
	def visit_Expresion(self, node):
		if(self.DeclarandoConstante):
			if(self.VisitandoModulo):
				...
				node.tipo="TYPE-CONST"
			target = self.new_node(node, shape='Box', color='yellow')
		else:
			target = self.new_node(node)
		self.dot.add_node(target)
	...
	def visit_ListaId(self, node):
		...
		if(self.VisitandoModulo):
			...
			if(self.PortsInOut):
				node.tipo="INOUT_Module"
				target = self.new_node(node, shape='octagon', color='orange')
			if(self.PortsOut):
				node.tipo="OUT_Module"
				target = self.new_node(node, shape='octagon', color='pink')
			if(self.PortsVar):
				node.tipo="VAR_Module"
				target = self.new_node(node, shape='octagon', color='violet')
		else:
			if(self.PortsIn):
				node.tipo="IN_TypeDeclaration"
				target = self.new_node(node, shape='octagon', color='lightgreen')
			if(self.PortsInOut):
				node.tipo="INOUT_TypeDeclaration"
				target = self.new_node(node, shape='octagon', color='orange')
			if(self.PortsOut):
				node.tipo="OUT_TypeDeclaration"
				target = self.new_node(node, shape='octagon', color='pink')
			if(self.PortsVar):
				node.tipo="VAR_TypeDeclaration"
				target = self.new_node(node, shape='octagon', color='violet')
				...
				if(field=="ID"):
					if(not self.VisitandoModulo):
						...
						if(self.StartAssing):
							for valuei in self.typeConst:
								if(valuei[0]==value):
									print(value,"es una Constante no se puede reasignarse")
									existValue=True
									targetHijo = self.new_node(None, label=value+" | "+"TYPE-CONST", shape='hexagon', color='yellow')
							for Ports in self.typePortsIn:
								if(Ports[0]==value):
						...
						for valuei in self.moduleConst:
							if(valuei[0]==value):
								print(value,"es una constante no se puede re asignar" )
								targetHijo = self.new_node(None, label=value+" | "+"MODULE-CONST", shape='hexagon', color='yellow')
						if(self.StartAssing):
							for Ports in self.modulePortsIn:
								if(Ports[0]==value):
