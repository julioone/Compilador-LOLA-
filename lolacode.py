...
class GenerateCode(ast.NodeVisitor):
	...
	def __init__(self):
		...
		# Una lista de declaracaiones externas (y tipos)
		self.externs = []
		self.stackBuild=[]
		self.stackExprOp=[]
		self.stackTermOp=[]
	...
	def visit_Modulo(self, node):
		...
		inst = ('Init_'+node.tipo,'name_' +node.ID1,'temporal' +target)
	...
	def visit_ListaId(self, node):
		...
		for IDs in node.idsListaId:
			target = self.new_temp(node.tipo)
			# Cree opcode SSA y agregelo a lista de instrucciones generadas
			inst = ('New_'+node.tipo,IDs,'temp'+target)
			self.code.append(inst)
	...
	def visit_TipoFormal(self, node):
		...
		node.gen_location = []
		for value in self.stackBuild:
			target = self.new_temp("Type")
			# Cree opcode SSA y agregelo a lista de instrucciones generadas
			inst = ('FormalType_'+value[1][2:-2], [value[1][2:], 'BIT'],'temp'+target)
			self.code.append(inst)
		...
		for field in getattr(node, "_fields"):
			...
			if isinstance(value, list):
				...
				for valueStack in self.stackBuild:
					target = self.new_temp("TAM")
					# Cree opcode SSA y agregelo a lista de instrucciones generadas
					inst = ('TAM_'+valueStack[1][2:-2], [valueStack[1][2:], "8"], target)
					self.code.append(inst)
	...
	def visit_TipoSimpleBasico(self, node):
		for field in getattr(node, "_fields"):
			...
			if value is not None:
				for valueStack in self.stackBuild:
					#print("yolo", value)
					target = self.new_temp("Temp_Type")
					# Cree opcode SSA y agregelo a lista de instrucciones generadas
					inst = ('Type_'+valueStack[1][2:-2], [valueStack[1][2:], str(value)], target)
					self.code.append(inst)
				self.stackBuild=[]
	...
	def visit_DeclaracionConstante(self, node):
		for field in getattr(node, "_fields"):
			...
			if isinstance(value, ast.AST):
				...
				target = self.new_temp(node.tipo)
				# Cree opcode SSA y agregelo a lista de instrucciones generadas
				inst = ('New_'+node.tipo, value,'temp'+target)
				self.code.append(inst)
				#self.stackBuild.append([value, target])

		targetBefore=target[2:]
		target = self.new_temp("CONST")
		# Cree opcode SSA y agregelo a lista de instrucciones generadas
		inst = ('Init_'+node.tipo, [targetBefore, self.valueVisit], target)
		self.code.append(inst)
	...
	def visit_Asignacion(self, node):
		...
		for enum, value in enumerate(self.stackValues):
			...
			if(value=="*"):
				...
				for operation in op:
					target = self.new_temp(""+binary_ops[operation])
					# Cree opcode SSA y agregelo a lista de instrucciones generadas
					inst = (binary_ops[operation]+'_'+node.tipo, [op1, firstResult],'temp' +target)
					self.code.append(inst)
					firstResult=target
					#self.stackBuild.append([value, target])
					try:
					...
					if(nextoperation is not None):
						...
						if(lastResult is None):
							...
							target = self.new_temp(""+binary_ops[opunary])
							inst = (binary_ops[opunary]+'_'+node.tipo, [lastResult, firstResult],'temp' +target)
							self.code.append(inst)
							lastResult=target
				...
				if(opunary is not None):
					target = self.new_temp(""+binary_ops[opunary])
					# Cree opcode SSA y agregelo a lista de instrucciones generadas
					inst = (binary_ops[opunary]+'_'+node.tipo, [lastResult, op1],'temp'+ target)
					self.code.append(inst)
					lastResult=target
					#firstResult=None
					op1=None
